...
class DuozhuayuCollector(BaseCollector):
    """多抓鱼采集器"""
    
    def __init__(self):
        super().__init__('duozhuayu')
    
    async def search_books(self, keyword: str, page: int = 1) -> List[BookData]:
        """搜索书籍"""
        search_url = f"{self.config.search_url}?keyword={keyword}&page={page}"
    ...
    async def parse_book_element(self, element) -> Optional[BookData]:
        ...
        try:
            ...
            # 生成书籍ID
            book_id = self.generate_book_id('duozhuayu', detail_url)
