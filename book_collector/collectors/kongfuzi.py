...
class KongfuziCollector(BaseCollector):
    """孔夫子旧书网采集器"""
    
    def __init__(self):
        super().__init__('kongfuzi')
    ...
    async def parse_book_element(self, element) -> Optional[BookData]:
        ...
        try:
            ...
            # 生成书籍ID
            book_id = self.generate_book_id('kongfuzi', detail_url)
            
            # 创建书籍数据对象
            book = BookData(
                book_id=book_id,
                title=title,
                author=None,  # 需要进入详情页获取
                publisher=publisher,
